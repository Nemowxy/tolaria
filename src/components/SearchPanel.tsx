...
const SearchInput = forwardRef<HTMLInputElement, SearchInputProps>(
  function SearchInput({ query, loading, onChange, onKeyDown, t }, ref) {
    return (
      <div className="flex items-center gap-3 border-b border-border px-4 py-3">
        ...
        <input
          ...
          aria-label={t('searchPanel.ariaSearchInput')}
          onKeyDown={onKeyDown}
        />
        {loading && (
          <svg
    ...
    <div className="flex-1 overflow-y-auto">
      {!query.trim() && (
        <div className="px-4 py-8 text-center">
          <p className="text-[13px] text-muted-foreground">{t('searchPanel.searchAcrossContents')}</p>
          <p className="mt-1 text-[11px] text-muted-foreground/60">
            {t('searchPanel.keyHint')}
          </p>
        </div>
      )}
      ...
      {query.trim() && results.length === 0 && !loading && (
        <div className="px-4 py-8 text-center">
          <p className="text-[13px] text-muted-foreground">{t('searchPanel.noResults')}</p>
        </div>
      )}
        ...
        <>
          <div className="border-b border-border/50 px-4 py-1.5">
            <span className="text-[11px] text-muted-foreground">
              {elapsedMs == null
                ? t('searchPanel.resultsSummary', { count: results.length })
                : t('searchPanel.resultsSummaryWithElapsed', { count: results.length, elapsedMs })}
            </span>
          </div>
          <div ref={listRef}>
