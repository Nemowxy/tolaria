...
function DisabledNavItem({
  ...
  return (
    <div className="flex select-none items-center gap-2 rounded text-foreground" style={{ padding, borderRadius: 4, opacity: 0.4, cursor: 'not-allowed' }} title={disabledTooltip ?? t('sidebarSections.comingSoon')}>
...
function InlineRenameInput({ initialValue, onSubmit, onCancel }: {
  ...
  return (
    <input
      ...
      aria-label={t('sidebarSections.sectionNameAria')}
...
function VisibilityPopoverItem({
  ...
  return (
    <Button
      ...
      aria-label={t('sidebarSections.toggleSection', { label })}
    >
...
export function VisibilityPopover({ sections, isSectionVisible, onToggle }: {
  ...
  return (
    ...
    >
      <div className="text-[12px] font-semibold text-muted-foreground" style={{ padding: '0 12px 4px' }}>{t('sidebarSections.showInSidebar')}</div>
      {sections.map((group) => (
